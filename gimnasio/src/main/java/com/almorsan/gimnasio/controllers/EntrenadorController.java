...
//    private EntrenadorService entrenadorService;
    ...
    public ResponseEntity<Entrenador> encontrarEntrenadorPorNick(@PathVariable String nick) {
        ...
        if (entrenador != null) {
            return ResponseEntity.ok(entrenador);  // Si el entrenador se encuentra, se devuelve
        } else {
            return ResponseEntity.notFound().build();  // Si no se encuentra, se devuelve un 404
        }
    }
